...
impl NetworkState {
    ...
    pub fn start_broadcast(&self) {
        ...
        self.mdns.register(my_service).expect("Failed to register mDNS service");
        println!("mDNS broadcasting as http://weddingsnap.local:3000 on IP {}", ip);
    }
}
...
pub fn ensure_firewall_rule() -> Result<(), String> {
    ...
    let check = Command::new("netsh")
        .args(["advfirewall", "firewall", "show", "rule", "name=WeddingSnap Server"])
        .output()
        .map_err(|e| format!("Failed to execute netsh: {}", e))?;

    if check.stdout.windows(b"WeddingSnap Server".len()).any(|w| w == b"WeddingSnap Server") {
...
#[tauri::command]
pub fn ping_router() -> bool {
    // We ping the sidecar port (3000). If this connects, we know the 
    // system is actively hosting the gallery to the local network!
